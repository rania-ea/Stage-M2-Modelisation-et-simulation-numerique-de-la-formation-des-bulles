...
eps = 1e-10
eta = 1e-8
alpha = 1 # le paramétre de relaxation ###ATTENTION il faut que le alpha soit petit 
relaxation = False
...
def domaine_ok(x):
    ...
    if not relaxation:
        return min(cAp, cBp, cAm, cBm, dp, dm) > eps and (cAp-cAm)**2 + (cBp-cBm)**2 > eta**2  #retourne un bool
    return min(cAp, cBp, cAm, cBm, dp, dm) > eps
...
def min_gradiant_adaptatif(x, relaxation):
    if relaxation:
        grad_J = jac_fonction_moindre_carre_relaxe
    else:
        grad_J = jac_fonction_moindre_carre
    ...
    c_np1 = c_n
    alpha_n = 1
    c_np1 =  c_n - alpha_n * grad_J(c_n)
    while not domaine_ok(c_np1):
        alpha_n = beta * alpha_n
        c_np1 = c_n -  alpha_n * grad_J(c_n)
    while np.linalg.norm(c_n - c_np1) > eps2 and n + 1 < nmax:
        c_n = c_np1
        n += 1
        alpha_n = 1
        c_np1 =  c_n - alpha_n * grad_J(c_n)
    ...
    if np.linalg.norm(c_n - c_np1) <= eps2:
        print("eps2")
    else:
        print("nmax")
    return c_np1
...
for j in range(N):
    ...
    res = min_gradiant_adaptatif(x0, relaxation = relaxation)
...
plt.legend()
if relaxation:
    plt.title("Solutions trouvées pour gradiant adaptatif relaxé")
else:
